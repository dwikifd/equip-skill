...
export async function installSkillForAgent(
  ...
  // Determine the target directory for the agent
  ...
  try {
    // Install DIRECTLY to the agent's folder (No .agents/ cache, No symlinks)
    ...
    return {
      success: true,
      path: agentDir,
      // No canonicalPath needed since we didn't use the cache
    };
  } catch (error) {
    return {
      success: false,
